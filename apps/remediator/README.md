...
un namespace de test éphémère** (distinct du namespace `dev` qu'il corrige réellement), puis
ouvre une Pull Request GitHub avec le manifest corrigé et le résultat du test. La PR doit être
relue et mergée par un humain — Argo CD applique ensuite le changement automatiquement.

## Contrat de sécurité : dev uniquement

Le remédiateur ne doit **jamais** agir sur `staging` ou `prod` — ces environnements ne se
promeuvent qu'à la main, après validation humaine de l'environnement précédent (voir
`docs/architecture.md` §7.3). Ce n'est pas qu'une convention : `main()` refuse explicitement de
s'exécuter si `TARGET_NAMESPACE` vaut `staging` ou `prod`.

## Ce que fait le test de staging automatique

Avant d'ouvrir la moindre PR, le script :
1. Déploie le manifest corrigé dans un namespace jetable (`remediator-staging-<id>`, à ne pas
   confondre avec le namespace persistant `staging`), jamais dans `dev` directement.
...
l'IA n'atteint jamais `dev` sans être passé par cette validation fonctionnelle au préalable
...
| `TARGET_NAMESPACE` | Namespace à corriger — doit rester `dev` (défaut). Refusé si `staging`/`prod`. |
| `MANIFEST_PATH` | Chemin du manifest dans le repo, défaut `apps/vulnerable-app/dev/deployment.yaml` |
